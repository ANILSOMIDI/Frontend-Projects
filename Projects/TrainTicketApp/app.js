...
function init () {
    ...
    e1("date").setAttribute("min", today);

    //Search form
    e1("searchForm").addEventListener("submit", handleSearch);

    //Proceed
    e1("proceedBtn").addEventListener("click", () => {
        bulidPassengersForm();
        e1("passengerSection").hidden = false;
        window.scrollTo({top: e1("passengerSection").offsetTop - 16, behavior: "smooth"});
    });

    //conform
    e1("conformBtn").addEventListener("click", conformBooking);

    //Download & Reset
    e1("downloadBtn").addEventListener("click", downloadTicket);
    e1("resetBtn").addEventListener("click", resetApp);
}

function handleSearch(e) {
    e.preventDefault();
    const from = fmt(e1("from").value);
    const to = fmt(e1("to").value);
    const date = fmt(e1("date").value);
    const cls = fmt(e1("class").value);
    const passengers = parseInt(e1("passengers").value || "1", 10);
...
function selectedTrain(train) {
    ...
    e1("selectedTrainName").textContent = `${train.name} (${train.id})`;
    e1("selectedDepature").textContent = train.depart;
    e1("selectedArrival").textContent = train.arrive;
    e1("selectedClass").textContent = state.selectedClass;

    e1("seatCount").value = state.seatCount;
    e1("seatCount").addEventListener("input", () => {
        const val = parseInt(e1("seatCount").value || "1", 10);
        state.seatCount = Math.max(1, Math.min(val, 6));
        renderSeatMap();
        updateFare();
    });

    e1("selectionSection").hidden = false;
    window.scrollTo({ top: e1("selectionSection").offsetTop - 16, behavior: "smooth"});
}

function renderSeatMap() {
    const seatMap = e1("seatMap");
    seatMap.innerHTML = "";
    const coaches = ["A1", "B2", "C3"];
    const row = 8;  //seats per row label
    const cols = 8;

    const booked = new setInterval(state.selectedTrain.bookedSeats);
...
function updateFare() {
    ...
    e1("baseFare").textContent = INR(base);
    e1("selectedSeatCount").textContent = state.selectedSeats.length;
    const total = base * state.selectedSeats.length;
    state.totalFare = total;
    e1("totalfare").textContent = INR(total);
    e1("proceedBtn").disabled = state.selectedSeats.length === 0;
}

function bulidPassengersForm() {
    const form = e1("passengerForm");
    form.innerHTML = "";
...
function conformBooking() {
    ...
    const inputs = e1("passengerForm").querySelectorAll("input, select");
    ...
    // Collect passeneger data
    state.passengersData = [];
    const rows = e1("passengerForm").querySelectorAll(".row");
    rows.forEach((row, idx) => {
        const [nameE1, ageE1, genE1] = row.querySelectorAll("input, select");
        state.passengersData.push({
            seat: state.selectedSeats[idx],
            name: nameE1.value.trim(),
            age: parseInt(ageE1.value, 10),
            gender: genE1.value
        });
    });

    renderSummary();
    e1("summarySection").hidden = false;
    window.scrollTo({ top: e1("summarySection").offsetTop - 16, behavior: "smooth"});
}
...
function renderSummary() {
    ...
    s.innerHTML = `
        ...
        <p><strong>Date:</strong> ${data.date}</p> - <strong>Class:</strong> ${data.cls}</p>
        <p><strong>Departure:</strong> ${data.depart}</p> - <strong>Arrival:</strong> ${data.arrive}</p>
        <hr>
        ...
        <p><strong>Total fare:</strong>Rs${INR(data.totalFare)}</p>
    `;
...
function downloadTicket() {
    const summary = e1("summary").innerHTML;
...
function resetApp() {
    ...
    e1("resultsSection").hidden = true;
    e1("selectionSection").hidden = true;
    e1("passengerSection").hidden = true;
    e1("summarySection").hidden = true;
